  ...
  const style = {
    borderTop: (index) =>
      active === index
        ...
        : { borderRadius: 10 },
    animate: (index) =>
      active === index ? { width: "100%" } : { width: "0%" },
    animateStyle: (index) =>
      active === index
  ...
  useEffect(() => {
    setSelketonTab(
      <div className="container-seklton-tab">
        <div className="seklton-content">
          <div className="seklton-tab" />
        </div>
        <div className="seklton-content">
          <div className="seklton-tab" />
        </div>
      ...
      <div className="container-spotlight">
        {loading ? (
          ...
          <Link
            href={
              posts[active]?.link
                ? `/article/${posts[active].link}`
                : `/article/${formtedTitle(posts[active].title)}`
            }
          ...
          {loading
            ...
            : posts
                ...
                .map((post, index) => (
                  ...
                  >
                    <Link href={ post.link ?`/article/${post.link}`: formtedTitle(`/article/${post.title}`) } className="content">
                      <div style={style.borderTop(index)} className="img">
                        <img
                          style={style.borderTop(index)}
                          src={post.meta}
                          alt={post.title}
                        />
                        <motion.div
